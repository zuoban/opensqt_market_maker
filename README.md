...
execution:
  ...
  max_gap_stack_slots: 0            # 默认关闭；1~10 为额外合并的已跨过漏买格数上限
```

`trading.order_quantity` 是每格目标金额，数量按合约精度取整后可能略有偏差。跳格合并默认关闭；如果旧配置中 `max_gap_stack_slots` 已是正数，升级后仍会保留该设置，需要显式改成 `0` 才关闭（环境变量 `OPENSQT_EXECUTION_MAX_GAP_STACK_SLOTS=0` 优先于 YAML）。

开启合并后，只计算同一行情连接中相邻两次有效规划之间向下跨过的中间格子，不包含下跌起点。下移一格仍买一格；下移三格、且中间两格确认漏买时，最多合并三格金额。启动、上涨、断流恢复及从未经过的上方空槽不会触发合并。未确认撤销或仍有持仓的格子不能补买；同格重试不增加格数，当前格成交后不再复用本次跌幅。已有挂单的数量不会因修改配置自动改变，需要重启加载新配置并按退出撤单设置处理旧单。
...
面板展示价格、K 线网格执行图、Maker 接受率与拒单/补格指标、程序启动后的成交订单、持仓、保证金占用和最近日志。保证金的人民币估值使用 Frankfurter 的 USD/CNY 日汇率：服务端每 6 小时异步刷新内存缓存，失败时每 10 分钟重试并保留最后一次成功值，不阻塞面板或交易流程。网格执行图会区分待确认、撤单中、重试冷却、状态异常与真正空网格，并在策略参数中显示订单容量已用/上限/剩余。默认只绑定本机；若改成 `0.0.0.0` 会对外暴露账户与仓位，请同时设置 `token`。
