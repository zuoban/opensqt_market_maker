...
经过数个版本迭代，我们已经使用此系统交易超过1亿美元的虚拟货币，例如，交易币安ETHUSDC，0手续，价格间隔1美元，每笔购买300美元，每天的交易量将达到300万美元以上，一个月可以交易5000万美元以上，只要市场是震荡或向上将持续产生盈利，如果市场单边下跌，3万美元保证金可以保证下跌1000个点不爆仓，通过不断交易拉低成本，只要回涨50%即可保本，涨回开仓原价可以赚到丰厚利润。
...
- **智能网格策略**: 
  ...
  - **固定网格执行**: 每格独立下单，保持原网格价格和金额；全路径保持 PostOnly，卖单 ReduceOnly。
- **安全边界**:
  - **资金安全**: 启动前自动检查余额、杠杆倍数与最大持仓风险。
  - **保证金硬限制**: 占用超过上限时锁存停单并全撤。
...
保存后重启程序生效。通知标题显示买卖方向和成交均价，例如 `卖出（SELL）｜ 均价 2500.25 USDC`；正文包含交易对、数量、成交金额、订单 ID 和带时区的成交时间。卖单还包含按交易所口径记录的已实现盈亏（未扣手续费）。只在系统新确认订单全部成交（`FILLED`）后通知，部分成交、撤销、拒绝和重复事件不会发送；对账确认的全成交也经过同一去重入口。
...
买单从当前网格下一格向下挂单，每张订单只使用 `trading.order_quantity` 对应的一格报价金额（数量按合约精度取整后可能略有偏差）。卖单按所属槽位价格加 `trading.price_interval` 提交，数量为该槽位实际持仓。行情跳格不会补买漏格、合并金额或拆仓，也不会把买卖委托价格重新挪到盘口附近。

所有订单仍强制 `PostOnly`，卖单仍强制 `ReduceOnly`。原价会立即成交时，Binance 可以拒绝该 PostOnly 单；程序保持原网格目标，按普通拒单冷却（1 秒）重新规划，不降级为 Taker。已有持仓不会因下单被拒而清空。买卖窗口大小仍决定可同时规划的订单数，并不限制短时间内连续成交的笔数。

原 `execution` 配置段及全部 `OPENSQT_EXECUTION_*` 环境变量已移除；升级后残留值会被忽略，可直接删除，不会恢复为原来的默认补格策略。部署修改需要重新构建并重启程序才能生效。

交易健康门禁继续检查唯一 WebSocket 行情、订单流、保证金和对账状态。每次实际提交仍核对槽位身份、数量及健康状态；批量大小只遵守交易所限制。下单结果为 UNKNOWN 时保留原槽位 `PENDING` 和原 ClientOrderID，通过订单流或对账确认，不换 ID 盲目重下。

重启后的已有仓位仍使用现有恢复规则；槽位卖出目标不等于真实成本保本价，移除执行配置不代表消除了旧仓亏损卖出的可能。
...
面板展示价格、K 线网格执行图、Maker 接受率与拒单指标、程序启动后的成交订单、持仓、保证金占用和最近日志。保证金的人民币估值使用 Frankfurter 的 USD/CNY 日汇率：服务端每 6 小时异步刷新内存缓存，失败时每 10 分钟重试并保留最后一次成功值，不阻塞面板或交易流程。网格执行图会区分待确认、撤单中、重试冷却、状态异常与真正空网格，并在策略参数中显示订单容量已用/上限/剩余。默认只绑定本机；若改成 `0.0.0.0` 会对外暴露账户与仓位，请同时设置 `token`。
