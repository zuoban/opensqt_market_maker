...
func TestNativeBatchDoesNotSplitOrdersByPriceDistance(t *testing.T) {
	ex := &batchRecordingExchange{batchSize: 5}
	executor := NewExchangeOrderExecutor(ex, "ETHUSDT", 0, 0)
	...
	placed, margin, err := executor.BatchPlaceOrders([]*OrderRequest{
		...
		{Symbol: "ETHUSDT", Side: "BUY", Price: 99, Quantity: 0.1, ClientOrderID: "near"},
	...
	if ex.batchCalls != 1 {
		t.Fatalf("native batch calls = %d, want one batch containing all three prices", ex.batchCalls)
	}
}
