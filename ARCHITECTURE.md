...
3. **批量操作**: 下单使用 Binance USD-M 原生批量接口，单批最多 5 笔，近盘口订单独占一批。每轮调整只规划并提交一批，其余槽位不预留；结果收敛后通知原串行协调器继续，重新检查健康与最新行情，ReduceOnly 卖单优先。撤单与全撤按 Binance 接口语义确认结果。
...
- 普通新开买单从当前网格下一格开始；当前格只在确认跳格漏买时作为合并父格下单。
...
- **盘口新鲜度**: 用同一条 combined 流最近一次成交或 bookTicker 判断静默；bookTicker 没变化不等于盘口失效。解析必须区分 b/B、a/A，避免 encoding/json 把数量覆盖成价格。若买一/卖一相对网格窗口完全不可达，回退到成交价安全垫
...
- **跳格合并**: 默认关闭，`execution.max_gap_stack_slots=0` 保持关闭；正数为额外合并格数上限（不含当前格）。只根据同一行情连接中相邻有效规划的网格价记录一次向下跨格区间，不包含下跌起点；仅中间确认漏买的连续格子可合并。启动、上涨、只下移一格和跨连接恢复均不增加买单金额。证据独立于规划指纹，同格撤单/拒单重试不累加格数；换格、行情不可用或当前格已有成交时失效。正在撤销的上方买单只允许触发当前格小额单的撤销，新合并买单须等上方撤销确认且无成交；提交 lease 按父槽到子槽的价格升序锁定并复检全部子槽，防止规划后迟到成交造成额外买入。成交后仍拆到对应槽位分别挂卖，既有持仓和卖单遵循原恢复逻辑。
- **版本化退避**: 同一盘口版本不重复尝试；`-5022` 默认按 50/100/200/400/500ms 退避，超过 burst 后至少等待 1 秒
- **批量隔离**: 原生批量请求使用同一原子盘口复检；近盘口订单拆成单笔提交。每轮规划上限由执行器的 `PlacementBatchSize()` 提供（Binance 为 5，无原生批量能力为 1）。达到批次边界后，下一轮按最新状态重新规划；UNKNOWN 交给门禁恢复，明确拒单保留既有冷却，不通过立即续批绕过。未提供该可选能力的兼容执行器保留原行为。
...
执行器通过 `OrderRequest.BeginSubmission` 把每次创建请求的释放边界传递给 Binance。初次单笔/原生批次仍先限流、锁住并复检槽位，再完成健康和 Maker 校验；锁保持到该次创建 HTTP 请求返回。原生批次的全部 lease 此时一起释放，随后按 ClientOrderID 的只读确认不再持有槽位锁。单笔、原生批量和兼容批量 wrapper 均转发此边界；未使用该可选协议的其它调用仍由外围释放兜底。

适配器内部的同 ID 重试必须再次调用此边界，重新限流并检查原新单上下文、槽位身份/数量、跳格子槽、健康状态和最新盘口。确认使用的独立只读上下文不能用于再次创建。复检失败返回 UNKNOWN，并立即关闭原新单代际；旧代际失败不会关闭已新建的代际。批次错误聚合必须优先保留 UNKNOWN，不能将其中的 `ErrOrderSubmissionStale` 当作普通未提交请求忽略。

仓位请求单独记录是否已经进入创建边界，不依赖性能观测是否开启。首次提交前失效仍可清理 reservation；已经进入创建边界后的重试失效必须先保留原身份与跳格子槽，等最终结果或对账收敛。确认期间允许订单流更新槽位，迟到 REST 返回继续经过原有终态去重、身份复检与库存前提检查：不能复活已成交订单，前提失效的已接受订单仍定向撤销并进入恢复。
