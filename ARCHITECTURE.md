...
3. **批量操作**: 下单使用 Binance USD-M 原生批量接口，单批最多 5 笔。每轮调整只规划并提交一批，其余槽位不预留；结果收敛后通知原串行协调器继续，重新检查健康与最新行情，ReduceOnly 卖单优先。撤单与全撤按 Binance 接口语义确认结果。
...
- `order_transition.go` 的 `reduceOrderUpdate` 接收槽位值副本、订单更新、已完成/终态进度、配置和时间，返回新槽位状态、会计增量、成交记录及补单意图。计算过程不访问管理器、锁、时钟或外部回调。
- `execution_transition.go` 统一部分成交、全成交及迟到终态的累计数量/金额、真实成本与盈亏补差计算。旧终态修正只更新库存和旧订单进度，保留当前绑定的新订单身份与记账字段；已接受的 REST 订单撤销路径复用同一成交算术。
- `order_transition_apply.go` 在读取时持有的同一映射槽位锁内发布结果，累计统计按增量合并。槽位解锁后才通知协调器，保留原 submission lease 和订单身份复检约束。
- 值副本只包含本次回调需要的字段，不复制 `InventorySlot` 的锁，也不覆盖其它拒单统计与创建时间元数据。它不是完整持久化模式；当前仍直接发布到内存，去重表持续增长问题尚未解决。
- 完整成交和终态历史查询保持 ClientOrderID 优先及原键格式，常规身份使用局部缓冲区完成查询；入表仍保存独立字符串。诊断日志在准备参数前检查等级，过滤不影响已发布的库存、累计盈亏或通知。

#### v3.5.14 恢复与记账边界

- 持仓恢复与正常买单复用数量量化，避免小金额除以高币价后舍入为零。已有仓位填入 `min(当前网格, 均价格)` 向下的买格，卖单仍挂在槽位价加一格；重启后不再把当前买窗当成空仓。恢复失败向启动流程传播；创建前校验有效数量、价格及成本，单次恢复最多 100,000 个槽位，超过时明确拒绝启动。
- 普通新开买单从当前网格下一格开始；当前格不新增买单，不补买漏格或合并金额。
...
- 卖价固定为所属槽位价加一格，不依据盘口移动价格；交易所决定 PostOnly 是否可受理。
- 每个持仓槽位仍保持一张独立 `ReduceOnly + PostOnly` 卖单，不合并数量。
- 已活跃、`PENDING/UNKNOWN` 和 `CANCEL_REQUESTED` 的 SELL 都持续占用其实际价格 tick；目标价被占用时，该槽等待已有订单收敛，不移动到其它 tick 或网格。
...
- 同格行情的跳过判断保存实际用于规划的盘口。只有 bid/ask、连接代际或就绪状态变化时，才因盘口更新重新规划；成交与冷却到期通知仍强制调整。

#### 只读面板的性能边界

- 有序索引保存槽位指针，成员增删以写时复制发布；扫描不再逐价查询 `sync.Map`。回收后标记旧槽 `retired`，旧视图跳过已回收指针；槽位内容仍在原锁内读写，下单前映射身份复检保持不变。
- 仓位快照利用有序索引预分配切片，字段复制后在槽位锁外格式化，最后线性反转为降序。
- 仓位管理器返回独占快照切片，缓存直接接管。面板内部只读组装共享缓存视图，外部 `View()` 仍返回副本；后续刷新不会修改已经发布的视图。
- 最近 20 条成交明细使用预分配的固定容量切片；满后在原成交记录锁内移除最旧记录并覆盖末尾。内部保持旧到新，快照在读锁内复制为最新优先的独立列表；展示截断不删除完整去重键，也不截断小时汇总。
- 缓存启动时生成首份快照；连续 10 秒无读取后暂停仓位扫描。HTTP/WS 再次读取只唤醒后台刷新，并返回带真实更新时间与年龄的缓存，绝不在请求线程等待槽位锁。性能观测及保证金守卫不受面板空闲影响。
- 一轮 WebSocket 广播在写协程中按需编码一次，多个连接共享 `PreparedMessage`；各连接仍独立执行写入、超时和单槽最新消息队列。
- WebSocket 可协商 `permessage-deflate`，使用 BestSpeed；同一广播的压缩帧也由 `PreparedMessage` 共享。不支持压缩的客户端继续使用原始 JSON，HTTP 快照协议保持不变。
- 基准、验证方法及仍保留的安全边界见 [PERFORMANCE.md](PERFORMANCE.md)。

#### 日志输出的性能边界

控制台和 DEBUG 文件各使用 4,096 条容量的独立异步队列。队列锁只保护入队与关闭，不覆盖控制台或磁盘 I/O；队列满时丢弃该输出并累计计数，最近 200 条面板日志仍同步记录。状态性能日志报告两类累计丢弃数。正常退出排空已入队输出；FATAL 同步输出到控制台，不因队列饱和丢失。

#### 只读性能观测

- 主程序为仓位管理器、订单执行器、协调器和对账器注入同一个 `telemetry.Recorder`，随主上下文启动、停止独立的每秒汇总协程。关闭面板时仍然采样并按状态打印间隔记录日志。
- 每项延迟保留最近 1,024 个样本；热点路径只尝试获取统计锁，竞争时跳过样本并累计 `droppedSamples`。排序、Go 运行时采样与状态计数都在后台执行，不增加行情或交易请求。
- 共 13 项延迟，记录规划、调整串行锁等待、限流、下单/撤单调用、订单回调与槽位锁等待，以及调整请求就绪后的调度等待、整轮对账耗时（含串行锁与 pending 收敛）。成交到反向提交使用本机回调入口时间，重复成交不重置；仅在所有门禁通过、持有原 submission lease 的首次真实提交边界结束计时。
- 每秒通过槽位原子只读标记统计待确认对向 BUY/SELL 的数量与最长等待，不获取槽位锁。它们从有效全成交开始，跨提交尝试、拒单和 UNKNOWN 保留，直到匹配订单被确认受理；槽位回收或库存修正使该方向不再需要时清除。不会为观测保留槽位；启动库存、部分成交和旧终态修正不创建起点。
...
`position/order_transition_ledger_test.go` 在测试中接通真实计算、同步持久提交与真实内存发布，覆盖部分成交、迟到终态、新订单绑定保留、盈亏补差和崩溃恢复；存储层另验证多槽写入原子性、精确十进制、并发与故障。离线 `GuardedSlots` 已通过真实执行器验证：存储故障后拦截新单，保护性撤单与 UNKNOWN 收敛仍可用。主程序依赖图仍不含账本或 bbolt，生产仍使用原内存去重表。完整状态模式、生产协调器接线、启动恢复、历史补齐与迁移尚待实现，具体边界见 [FILL_LEDGER_DESIGN.md](FILL_LEDGER_DESIGN.md)。
...
- 每轮先收敛待确认订单，再并行查询远端持仓与挂单，共享 10 秒截止时间；任一查询失败会取消另一项并保持不健康。对账轮次及本地校验仍然串行，不在查询期间放行新的订单调整。
- 默认每 60 秒（可配置）
- 启动放行前同步执行一次
- 订单流异常恢复后强制执行一次
...
- **分类重试**: 仅重试明确可安全重试的失败
- **严格 PostOnly**: 所有网格单只做 Maker，卖单强制 ReduceOnly；原网格价被行情穿过时也不改变目标价，不降级为普通单
- **行情健康**: 唯一 combined 流的成交和 bookTicker 就绪状态由统一健康门禁检查；行情过期、订单流异常或对账失败会停止新单
- **固定金额与价格**: 每张买单只买一格金额；卖单按所属槽位价加一格、实际槽位数量提交。跳格不加量、不补格、不拆仓，不在盘口附近重新定价
- **配置迁移**: `execution` 段及全部 `OPENSQT_EXECUTION_*` 已移除，残留设置被忽略，不回退到旧执行默认值
- **普通拒单冷却**: PostOnly 拒单与其它明确未受理请求共用 1 秒冷却，不再使用专用指数退避或盘口版本门槛
- **批量提交**: 每轮规划上限由执行器的 `PlacementBatchSize()` 提供（Binance 为 5，无原生批量能力为 1），不按距离盘口远近拆批。下一轮按最新健康和库存重新规划；UNKNOWN 交给门禁恢复，明确拒单保留冷却。未提供该可选能力的兼容执行器保留原行为。
...
执行器通过 `OrderRequest.BeginSubmission` 把每次创建请求的释放边界传递给 Binance。初次单笔/原生批次仍先限流、锁住并复检槽位，再完成交易健康校验；锁保持到该次创建 HTTP 请求返回。原生批次的全部 lease 此时一起释放，随后按 ClientOrderID 的只读确认不再持有槽位锁。单笔、原生批量和兼容批量 wrapper 均转发此边界；未使用该可选协议的其它调用仍由外围释放兜底。

适配器内部的同 ID 重试必须再次调用此边界，重新限流并检查原新单上下文、槽位身份/数量和交易健康状态。确认使用的独立只读上下文不能用于再次创建。复检失败返回 UNKNOWN，并立即关闭原新单代际；旧代际失败不会关闭已新建的代际。批次错误聚合必须优先保留 UNKNOWN，不能将其中的 `ErrOrderSubmissionStale` 当作普通未提交请求忽略。

仓位请求单独记录是否已经进入创建边界，不依赖性能观测是否开启。首次提交前失效仍可清理 reservation；已经进入创建边界后的重试失效必须先保留原身份，等最终结果或对账收敛。确认期间允许订单流更新槽位，迟到 REST 返回继续经过原有终态去重、身份复检与库存前提检查：不能复活已成交订单，前提失效的已接受订单仍定向撤销并进入恢复。
